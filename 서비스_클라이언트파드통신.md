...
https://kubernetes.io/ko/docs/concepts/services-networking/_print/#pg-5701136fd2ce258047b6ddc389112352
## 서비스
...
#### 레디니스 프로브 소개   
라이브니스 프로브는 불완전한 컨테이너를 자동으로 다시 시작해 애플리케이션의 상태를 원활히 유지하는 방법이었다.   
레디니스 프로브는 주기적으로 호출되며 특정 파드가 클라이언트 요청을 수신할 수 있는지를 결정한다.   
   
- **레디니스 프로브 유형**   
  레디니스 프로브는 라이브니스 프로브와 똑같이 세 가지 유형의 프로브가 있다.   
    - HTTP GET 프로브: HTTP GET 요청을 컨테이너로 보내고 응답의 HTTP 상태 코드를 보고 컨테이너가 준비됐는지 여부를 결정한다.   
    - TCP 소켓 프로브: 컨테이너의 지정된 포트로 TCP 연결을 연다. 소켓이 연결되면 컨테이너가 준비된 것으로 간주한다.   
    - Exec 프로브: 컨테이너의 상태를 프로세스의 종료 상태 코드로 결정한다.   
      
- **레디니스 프로브의 동작**   
  라이브니스 프로브와 달리 컨테이너가 준비 상태 점검에 실패하더라도 컨테이너가 종료되거나 다시 시작되지 않는다.   
  이는 라이브니스 프로브와 레디니스 프로브 사이의 중요한 차이이다.   
  다음 그림에서 보듯이 파드의 레디니스 프로브가 실패하면 파드는 엔드포인트 오브젝트에서 제거된다. 서비스로 연결하는 클라이언트의 요청은 파드로 전달되지 않는다.   
![스크린샷, 2021-06-18 18-37-47](https://user-images.githubusercontent.com/49184890/122541225-5bc6e300-d064-11eb-8bda-3b5dcac2b652.png)


#### 파드에 레디니스 프로브 추가   
다음으로 rc의 파드 템플릿을 수정해 기존 파드에 레디니스 프로브를 추가한다.   
- **파드 템플릿에 레디니스 프로브 추가**   
  ~~~
  $ kubectl edit rc kubia
  ~~~
  YAML이 열리면  spec.template.spec.containers에 아래의 컨테이너에 다음과 같이 레디니스 프로브를 추가한다.   
  ~~~
  apiVersion: v1
  ...
  즉, 파일이 있으면 레디니스 프로브가 성공하고 그렇지 않으면 실패한다.   
  아래의 명령어로 ready 파일을 생성하면 레디니스 프로브가 호출되는 순간 해당 파드가 준비될 것이다.
  ~~~
...
> Tip: 서비스에서 파드를 수동으로 추가하거나 제거하려면 파드와 서비스의 레이블 셀렉터에 enabled=true 레비을을 추가한다. 서비스에서 파드를 제거하려면 레이블을 제거하라.   
#### 레디니스 프로브에서 중요한 두 가지 사항
- 1. 레디니스 프로브를 항상 정의하라.   
- 2. 레디니스 프로브에 파드의 종료 코드를 포함하지 마라.   
### 헤드리스 서비스로 개별 파드 찾기
클라이언트가 모든 파드에 연결해야 하는 경우에는 클라이언트가 DNS 조회로 파드 IP를 찾을 수 있도록 한다.   
일반적으로 서비스에 대한 DNS 조회를 수행하면 DNS 서버는 하나의 (서비스의)클러스터 IP를 반환한다.   
그러나 서비스에 클러스터 IP가 필요하지 않다면 (서비스 spec에서 clusterIP 필드를 None으로 설정해 이를 수행하면) DNS 서버는 하나의 서비스 IP 대신 여러 개의 파드 IP들을 반환한다.   
...
#### DNS로 파드 찾기
DNS 조회는 파드 내에서 수행한다.   
DNS 조회를 수행하기 위해 **nslookup 및 dig 바이너리**를 포함하는 컨테이너 이미지를 사용해야 한다.
다음과 같이 YAML을 생성하지 않고 파드를 실행한다.   
~~~
